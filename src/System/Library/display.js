...
globalThis.Window = class{
  ...
  getParams(){
    return {startx: startx, starty: starty, x: x, y: y, isVisible: isVisible, win: win}
  }
  ...
  render(){
    ...
    for(let j = 0; j < this.x; j++){
      for(let z = 0; z < this.y; z++){
        if(inBounds(this.startx+j,0,config.displayx) && inBounds(this.starty + z, 0, config.displayy)){
